...
(async function createSpotitHero() {
  ...
  try {
    ...
    function token(name,value,type,scopes,syntax){
      ...
      if(!raw){raw=figma.variables.createVariable(name,primitives,type);raw.setValueForMode(primitives.defaultModeId,value);raw.scopes=[];raw.setVariableCodeSyntax('WEB',`var(--spotit-hero-raw-${name.replaceAll('/','-')})`);}
    ...
    function label(parent,name,str,styleName,color='ink',width){
      const n=mark(figma.createText());parent.appendChild(n);n.name=name;n.fontName=regular;n.characters=str;n.textStyleId=styles[styleName].id;n.fills=[paint(color)];
    ...
    for(const c of SPEC.cards){
      ...
      const comp=mark(figma.createComponent());library.appendChild(comp);comp.name=`Size=${c.variant}`;comp.resize(c.width,c.height);comp.layoutMode='VERTICAL';comp.primaryAxisSizingMode='FIXED';comp.counterAxisSizingMode='FIXED';padding(comp,c.padding);gap(comp,c.gap);comp.fills=[paint('paper')];comp.strokes=[paint('line')];comp.strokeWeight=.6;corners(comp,large?22:20);comp.effectStyleId=cardEffect.id;
