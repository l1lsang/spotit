...
**지도 탭의 그룹 아이콘**을 누르면 그룹 페이지(`/groups`)에서 공개 그룹을 검색하거나 내 그룹을 볼 수 있습니다. 그룹 생성 시 공개·비공개를 선택하며 개설자는 자동 가입됩니다. 공개 그룹은 로그인 사용자가 둘러볼 수 있고, 비공개 그룹은 가입한 멤버의 **내 그룹**에만 표시됩니다. 두 유형 모두 **초대코드로 가입**하며, 그룹 홈에서 멤버가 12자리 코드를 확인·복사할 수 있습니다. 코드의 대소문자·공백·하이픈은 구분하지 않습니다. 공유 링크만으로는 가입되지 않습니다.
...
**운영 반영 순서:** 기존 데이터가 있다면 Admin SDK의 Application Default Credentials로 아래 스크립트를 먼저 실행해 `visibility`가 없는 기존 그룹을 `public`으로 갱신한 뒤, 새 앱과 `firestore.rules`를 함께 배포합니다. 스크립트는 이미 설정된 공개 범위와 멤버십·핀을 변경하지 않습니다. 기존 그룹의 초대코드는 멤버가 그룹 홈을 처음 열 때 발급됩니다. 데이터 갱신 전에도 기존 멤버는 내 그룹에서 접근할 수 있지만, 공개 목록에는 갱신 후 표시됩니다.
