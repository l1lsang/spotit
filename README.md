...
하단 **그룹** 메뉴(`/groups`)에서 그룹을 검색하거나 내 그룹을 볼 수 있습니다. 가입을 완료한 사용자라면 누구나 그룹을 만들 수 있으며, 개설자는 자동 가입됩니다. 별도 승인 없이 **바로 가입**으로 참여하고 그룹별 지도·목록에서 함께 모은 핀을 확인합니다.

그룹 지도에서 장소를 검색하거나 길게 눌러 핀을 남깁니다. 일반 지도에서 작성할 때도 **핀을 모을 곳**에서 가입한 그룹을 선택할 수 있습니다. 그룹 핀은 모든 로그인 사용자에게 공개되며, 등록·다른 그룹으로 이동하려면 해당 그룹의 멤버여야 합니다. 탈퇴 후에도 기존 핀은 유지되고 작성자가 수정·삭제할 수 있습니다.
