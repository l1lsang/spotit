...
**지도 탭의 그룹 아이콘**을 누르면 그룹 페이지(`/groups`)에서 공개 그룹을 검색하거나 내 그룹을 볼 수 있습니다. 그룹 생성 시 공개·비공개를 선택하며 개설자는 자동 가입됩니다. 공개 그룹은 로그인 사용자가 둘러보고 초대코드 없이 **바로 가입**할 수 있습니다. 비공개 그룹은 가입한 멤버의 **내 그룹**에만 표시되며 **초대코드로 가입**합니다. 비공개 그룹 홈에서 멤버가 12자리 코드를 확인·복사할 수 있습니다. 코드의 대소문자·공백·하이픈은 구분하지 않습니다. 비공개 그룹은 공유 링크만으로 가입할 수 없습니다. 기존에 발급된 공개 그룹 코드도 계속 사용할 수 있습니다.

그룹 홈의 **지도에서 핀 남기기**나 **지도에서 모아보기**를 누르면 `/map?group={groupId}`로 이동하고 해당 그룹만 지도에 표시됩니다. 내 지도에서 작성할 때도 **핀을 모을 곳**에서 가입한 그룹을 선택할 수 있습니다. 공개 그룹 핀은 로그인 사용자에게 공개되고, 비공개 그룹 핀은 `visibility: 'group'`으로 저장하여 그룹 멤버만 조회합니다. 비공개 핀은 다른 사람의 프로필 앨범·팔로워 피드에 섞이지 않습니다. 등록·다른 그룹으로 이동하려면 대상 그룹의 멤버여야 합니다. 탈퇴 후에는 다른 멤버의 비공개 핀 접근이 차단되지만, 본인이 남긴 핀은 유지되며 수정·삭제할 수 있습니다. 생성한 그룹의 공개 범위는 고정입니다.

`groups/{groupId}`, `groups/{groupId}/members/{uid}`, `users/{uid}/groupMemberships/{groupId}`에 그룹과 가입 정보를 저장합니다. 코드는 멤버만 읽는 `groups/{groupId}/private/invite`에 보관합니다. `groupInvites/{code}`는 코드를 알고 있는 사용자의 단건 조회만 허용하며 목록 조회·변조를 차단합니다. 가입·탈퇴와 회원 수는 원자적으로 갱신합니다. Firestore 쿼리 자체를 공개 그룹 또는 특정 멤버십으로 제한합니다([공식 쿼리 권한 문서](https://firebase.google.com/docs/firestore/security/rules-query)).

**운영 반영 순서:** 기존 데이터가 있다면 Admin SDK의 Application Default Credentials로 아래 스크립트를 먼저 실행해 `visibility`가 없는 기존 그룹을 `public`으로 갱신한 뒤, 새 앱과 `firestore.rules`를 함께 배포합니다. 스크립트는 이미 설정된 공개 범위와 멤버십·핀을 변경하지 않습니다. 초대코드가 없는 비공개 그룹은 멤버가 그룹 홈을 처음 열 때 코드가 발급됩니다. 데이터 갱신 전에도 기존 공개 그룹은 코드 없이 가입할 수 있고, 기존 멤버는 내 그룹에서 접근할 수 있지만, 공개 목록에는 갱신 후 표시됩니다.

```bash
node functions/scripts/migrate-groups.js --project PROJECT_ID
node functions/scripts/migrate-groups.js --project PROJECT_ID --apply
firebase deploy --only firestore:rules
firebase deploy --only functions:submitModerationCase
```

`npm run test:groups:integration`으로 Firebase Auth·Firestore 에뮬레이터에서 생성·코드 검증·동시 가입·탈퇴·비공개 그룹/핀 접근·권한 우회 차단·기존 데이터 갱신과 핀 테마 호환성을 확인합니다. Firebase CLI와 Java 21 이상이 필요합니다.
