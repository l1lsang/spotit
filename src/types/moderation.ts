export type CaseKind = 'pin' | 'user' | 'chat' | 'inquiry' | 'auto'
export type CaseStatus = 'open' | 'reviewing' | 'resolved' | 'dismissed'
export const CASE_LABELS: Record<CaseKind, string> = { pin: '핀 신고', user: '유저 신고', chat: '채팅 신고', inquiry: '문의', auto: '자동 검토' }
...
export interface ReportTarget { kind: 'pin' | 'user' | 'chat'; targetId: string; label: string; messageId?: string }
