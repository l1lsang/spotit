...
function UsersPanel({ token, onChanged }: { token: string; onChanged: () => Promise<void> }) {
  ...
  return <><section className="admin-panel"><form className="admin-user-search" onSubmit={e => { e.preventDefault(); setQuery({ field, search: search.trim(), version: query.version + 1 }) }}><label><span>검색 항목</span><select value={field} onChange={e => setField(e.target.value)} disabled={loading}><option value="nickname">닉네임</option><option value="username">사용자 이름</option><option value="email">이메일</option><option value="uid">UID</option></select></label><label className="admin-search-input"><span>{field === 'email' || field === 'uid' ? '정확한 값 입력' : '시작하는 글자로 검색'}</span><input value={search} onChange={e => setSearch(e.target.value)} maxLength={200} placeholder="비워 두면 전체 유저를 보여 드려요" disabled={loading} /></label><button className="button button-primary" disabled={loading}><Search size={17} />검색</button></form>
    {error && <p className="form-error" role="alert">{error}</p>}
    <div className="admin-table-scroll"><table className="admin-user-table"><thead><tr><th>유저</th><th>이메일</th><th>가입일</th><th>상태</th><th>관리</th></tr></thead><tbody>{users.map(user => <tr key={user.uid}><td><div className="admin-user-identity">{user.photoURL ? <img src={safeImage(user.photoURL)} alt="" /> : <span>{user.nickname.slice(0, 1) || 'S'}</span>}<div><strong>{user.nickname || '이름 없음'}</strong><small>@{user.username || '미설정'}</small></div></div></td><td>{user.email || '이메일 없음'}</td><td>{user.createdAt ? new Date(user.createdAt).toLocaleDateString('ko-KR') : '기록 없음'}</td><td><span className={`case-badge ${user.suspended ? 'dismissed' : 'resolved'}`}>{user.suspended ? '이용 정지' : '정상'}</span></td><td><button className="button button-secondary" onClick={() => setSelected(user)} aria-label={`${user.nickname} 관리`}>관리</button></td></tr>)}</tbody></table></div>
...
function UserDetail({ user, token, onClose, onSaved }: { user: AdminUser; token: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <AdminDialog title="유저 상세·관리" busy={busy} onClose={onClose}><h3>{user.nickname} <small>@{user.username}</small></h3><dl className="admin-user-details"><dt>UID</dt><dd>{user.uid}</dd><dt>이메일</dt><dd>{user.email || '없음'}</dd><dt>가입일</dt><dd>{date(user.createdAt)}</dd><dt>상태</dt><dd>{user.suspended ? '이용 정지' : '정상'}</dd><dt>소개</dt><dd className="preserve-text">{user.bio || '없음'}</dd>{user.suspensionReason && <><dt>최근 처리 사유</dt><dd>{user.suspensionReason}</dd></>}</dl><form className="form" onSubmit={submit}><label>{user.suspended ? '정지 해제' : '이용 정지'} 사유<textarea rows={3} required maxLength={500} value={reason} onChange={e => setReason(e.target.value)} disabled={busy} placeholder="처리 근거를 남겨 주세요." /></label><p className="report-notice">{user.suspended ? '해제하면 다시 로그인해 서비스를 이용할 수 있습니다.' : '정지하면 로그인이 차단되고 기존 로그인 세션에서도 핀·채팅을 이용할 수 없습니다.'}</p>{error && <p className="form-error" role="alert">{error}</p>}<button className={`button ${user.suspended ? 'button-primary' : 'button-danger'}`} disabled={busy || !reason.trim()}>{busy ? '처리 중…' : user.suspended ? '이용 정지 해제' : '이용 정지 적용'}</button></form></AdminDialog>
}
