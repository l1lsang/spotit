...
export function ReportDialog({ target, onClose }: { target: ReportTarget; onClose: () => void }) {
  ...
  return (
    <dialog ref={dialog} className="report-dialog" onCancel={event => { if (busy) event.preventDefault(); else onClose() }} onClose={onClose} aria-labelledby={`${formId}-title`}>
      <div className="modal-header">
        <div className="report-heading">
          <div className="moderation-heading-icon"><Flag size={22} aria-hidden="true" /></div>
          <div><p className="eyebrow">안전한 스팟잇</p><h2 id={`${formId}-title`}>{target.kind === 'pin' ? '핀' : target.kind === 'user' ? '유저' : '채팅'} 신고</h2></div>
        </div>
        ...
        <form className="form" onSubmit={submit} aria-busy={busy}>
          ...
          <label className="field">
            ...
            <select required value={reason} onChange={e => setReason(e.target.value)} disabled={busy}>
              ...
              {REPORT_REASONS.filter(r => target.kind === 'pin' || r.id !== 'false_place').map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
            </select>
          </label>
          <label className="field">
          ...
          <div className="report-notice">
            ...
            <p>신고자 정보는 상대에게 공개되지 않습니다.{target.kind === 'chat' ? target.messageId ? ' 선택한 메시지와 첨부 사진이 검토를 위해 전달됩니다.' : ' 최근 메시지 최대 20개와 첨부 사진이 검토를 위해 전달됩니다.' : ' 신고 당시 콘텐츠가 검토를 위해 보관됩니다.'}</p>
          </div>
